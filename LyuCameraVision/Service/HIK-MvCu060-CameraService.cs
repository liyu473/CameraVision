...
public class HIK_MvCu060_CameraService
{
    ...
    public bool InitializeSdk()
    {
        ...
        {
            _logger.LogError("初始化相机SDK失败:{Exception}", ex);
            return false;
        }
    }
    ...
    {
        SDKSystem.Finalize();
        _logger.LogInformation("海康SDK已释放");
    }
    ...
    public List<IDeviceInfo> EnumerateDevices()
    {
        ...
        {
            _logger.LogError("枚举设备失败! 错误码: {ErrorCode:X}", nRet);
            return [];
        }

        _logger.LogInformation("枚举到 {DeviceCount} 个设备", deviceInfoList.Count);
    ...
    public bool OpenCamera(IDeviceInfo camera)
    {
        ...
        {
            _logger.LogError("创建设备失败!:{Exception}", ex);
            return false;
        }
        ...
        {
            _logger.LogError("打开设备失败! 错误码: {ErrorCode:X}", result);
            return false;
        }
        ...
        if (device is IGigEDevice gigEDevice)
        {
            ...
            if (result != MvError.MV_OK)
            {
                _logger.LogWarning("获取最佳包大小失败! 错误码: {ErrorCode:X}", result);
            }
            else
            {
                ...
                if (result != MvError.MV_OK)
                {
                    _logger.LogWarning("设置包大小失败! 错误码: {ErrorCode:X}", result);
                }
                else
                {
                    _logger.LogInformation("GigE设备包大小设置为: {PacketSize}", optionPacketSize);
                }
            }
        }
        ...
        _logger.LogInformation("相机打开成功: {ModelName}", camera.ModelName);
        return true;
    }
    ...
    public bool CloseCamera()
    {
        ...
        {
            _logger.LogError("关闭设备失败! 错误码: {ErrorCode:X}", ret);
            return false;
        }
    ...
    public bool StartGrabbing()
    {
        ...
        if (result != MvError.MV_OK)
        {
            _logger.LogError("开始采集失败! 错误码: {ErrorCode:X}", result);
            return false;
        }

        IsGrabbing = true;
        _logger.LogInformation("开始采集成功");
        return true;
    }
    ...
    public bool StopGrabbing()
    {
        ...
        if (result != MvError.MV_OK)
        {
            _logger.LogError("停止采集失败! 错误码: {ErrorCode:X}", result);
            return false;
        }

        IsGrabbing = false;
        _logger.LogInformation("停止采集成功");
        return true;
    }
    ...
    public bool SetTriggerMode(bool isTriggerMode)
    {
        if (device == null)
        {
            _logger.LogError("设备为空，无法设置触发模式");
            return false;
        }
        ...
        if (result != MvError.MV_OK)
        {
            _logger.LogError("设置触发模式失败! 错误码: {ErrorCode:X}", result);
            return false;
        }

        _logger.LogInformation("触发模式设置为: {Mode}", mode);
        return true;
    }
    ...
    public bool SetTriggerSource(bool isSoftTrigger)
    {
        ...
        if (result != MvError.MV_OK)
        {
            _logger.LogError("设置触发源失败! 错误码: {ErrorCode:X}", result);
            return false;
        }

        _logger.LogInformation("触发源设置为: {Source}", source);
        return true;
    }
    ...
    public bool TriggerSoftware()
    {
        ...
        if (result != MvError.MV_OK)
        {
            _logger.LogError("软触发执行失败! 错误码: {ErrorCode:X}", result);
            return false;
        }

        _logger.LogInformation("软触发执行成功");
        return true;
    }
    ...
    public bool GetExposureTime(out float exposureTime)
    {
        ...
        if (device == null)
        {
            _logger.LogError("设备为空，无法获取曝光时间");
            return false;
        }
        ...
        _logger.LogError("获取曝光时间失败! 错误码: {ErrorCode:X}", result);
        return false;
    }
    ...
    public bool GetGain(out float gain)
    {
        ...
        _logger.LogError("获取增益失败! 错误码: {ErrorCode:X}", result);
        return false;
    }
    ...
    public bool GetFrameRate(out float frameRate)
    {
        ...
        _logger.LogError("获取帧率失败! 错误码: {ErrorCode:X}", result);
        return false;
    }
    ...
    public bool SetExposureTime(float exposureTime)
    {
        if (device == null)
        {
            _logger.LogError("设备为空，无法设置曝光时间");
            return false;
        }
        ...
        if (result != MvError.MV_OK)
        {
            _logger.LogError("设置曝光时间失败! 错误码: {ErrorCode:X}", result);
            return false;
        }

        _logger.LogInformation("曝光时间设置为: {ExposureTime} us", exposureTime);
        return true;
    }
    ...
    public bool SetGain(float gain)
    {
        ...
        if (result != MvError.MV_OK)
        {
            _logger.LogError("设置增益失败! 错误码: {ErrorCode:X}", result);
            return false;
        }

        _logger.LogInformation("增益设置为: {Gain}", gain);
        return true;
    }
    ...
    public bool SetFrameRate(float frameRate)
    {
        ...
        if (result != MvError.MV_OK)
        {
            _logger.LogWarning("启用帧率控制失败! 错误码: {ErrorCode:X}", result);
        }

        result = device.Parameters.SetFloatValue("AcquisitionFrameRate", frameRate);
        if (result != MvError.MV_OK)
        {
            _logger.LogError("设置帧率失败! 错误码: {ErrorCode:X}", result);
            return false;
        }

        _logger.LogInformation("帧率设置为: {FrameRate} fps", frameRate);
        return true;
    }
    ...
    private bool SaveImage(ImageFormatType formatType, string? filePath = null, int jpegQuality = 80)
    {
        ...
        lock (saveImageLock)
        {
            ...
            try
            {
                ...
                if (result != MvError.MV_OK)
                {
                    _logger.LogError("保存图像失败! 错误码: {ErrorCode:X}", result);
                    return false;
                }

                _logger.LogInformation("图像保存成功: {FilePath}", filePath);
                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError("保存图像异常: {Message}", ex.Message);
                return false;
            }
        }
    }
